...
#SBATCH --mail-user=victorialang2027@u.northwestern.edu
#SBATCH --mail-type=END
#SBATCH --time=30-00:00:00
#SBATCH --mem-per-cpu=4G
#SBATCH --job-name=Win23D02
#SBATCH -n 200
...
set option  = 3		#model-run options, 
...
set WRF_DIR     = $WORKDIR/BLD_WRFrelease-v4.5.1_CCTM_v55_gcc_main # Location of WRF-CMAQ Install
set INPDIR      = /projects/b1045/rcs_support/wrf-cmaq/WRF_DIR/work_dir/WRFv451_CMAQv55/data/202301_baseline_d02             # Input directory for WRF & CMAQ
set OUTPUT_ROOT = $WORKDIR                                # output root directory
set output_direct_name = WRFCMAQ-BASE_202301_D02-${version}        # Output Directory Name
...
set START_DATE = "2023-01-01"     # beginning date (July 1, 2016)
set END_DATE   = "2023-02-02"     # ending date    (July 14, 2016)
...
set resolution = 4000            # domain resolution in meter
...
setenv verbose_rdbcon T		      # added by DBP
...
setenv GRID_NAME ChicagoLADCO_d02         # check GRIDDESC file for GRID_NAME options
setenv GRIDDESC /projects/b1045/rcs_support/wrf-cmaq/output/Chicago_LADCO/mcip_output/daily/ChicagoLADCO_202301_d02/GRIDDESC   # grid description file

#> WRF-CMAQ number of columns, rows and layers 
setenv WRF_COL_DIM        445   # wrf west_east_stag
setenv WRF_ROW_DIM        421   # wrf south_north_stag
setenv WRF_LAY_DIM         36   # wrf bottom_top_stag

setenv CMAQ_COL_DIM       432   # CMAQ Domain Columns
setenv CMAQ_ROW_DIM       408   # CMAQ Domain Rows
...
setenv CTM_MINSYNC           1   #> min sync time step (sec) [ default: 60 ]
...
setenv CTM_WB_DUST           Y   #> use inline windblown dust emissions [ Y ]
...
setenv CTM_STAGE_P22 Y       #> Pleim et al. 2022 Aerosol deposition model [default: N]
...
setenv LTNGDIAG                Y  #> lightning diagnostic file [ N ]
...
set IN_LTpath = $INPDIR/emis/lightning                  #> lightning NOx input directory
set METpath   = $INPDIR/met/wrfv4.5.1_inputs        #> meteorology input directory 
...
#set LUpath    = $INPDIR/met/mcipv5.4
# =====================================================================
...
while ($TODAYJ <= $STOP_DAY )  #>Compare dates in terms of YYYYJJJ
  ...
  set YYYYMMDD_2 = `date -ud "${YYYYMMDD}" +%Y-%m-%d` #> Convert YYYYMMDDYYYY-MM-DD to YYYYMMDD
...
# =====================================================================
  ...
  if ($NEW_START == true || $NEW_START == TRUE ) then
     setenv ICFILE ICON.20221223.4CHICAGOD2.base2023_rxeqemis.12US1CMAQ55.ncf
  ...
  #> Boundary conditions
  set BCFILE = BCON.${YYYYMMDD}.4CHICAGOD2.base2023_rxeqemis.12US1CMAQ55.ncf
  ...
  setenv CMAQ_MASKS $SZpath/OCEAN_12_L3m_MC_CHL_chlor_ChicagoLADCO_d02_windowed_from_12US1.ncf3 #> horizontal grid-dependent ocean file - includes DMS and CHLO vars - DBP
  #setenv verbose_rdbcon T  #added by DBP
  #> Gridded Emissions Files 
  setenv N_EMIS_GR 1		#number of Gridded Streams - DBP
  set EMISfile  = emis_mole_all_${YYYYMMDD}_ChicagoLADCO_d02_nobeis_withrwc_2022hc_cb6_22m.ncf
  ...
  set STKCASEG = ChicagoLADCO_d02_2022hc_cb6_22m              # Stack Group Version Label
  set STKCASEE = ChicagoLADCO_d02_cmaq_cb6ae7_2022hc_cb6_22m  # Stack Emission Version Label

  # Time-Independent Stack Parameters for Inline Point Sources
  setenv STK_GRPS_001 $IN_PTpath/stack_groups_ptnonipm_${STKCASEG}.ncf
  setenv STK_GRPS_002 $IN_PTpath/stack_groups_ptegu_${STKCASEG}.ncf
  setenv STK_GRPS_003 $IN_PTpath/stack_groups_ptagfire_${YYYYMMDD}_${STKCASEG}.ncf
  setenv STK_GRPS_004 $IN_PTpath/stack_groups_ptfire-rx_${YYYYMMDD}_${STKCASEG}.ncf
  setenv STK_GRPS_005 $IN_PTpath/stack_groups_ptfire_othna_${YYYYMMDD}_${STKCASEG}.ncf
  setenv STK_GRPS_006 $IN_PTpath/stack_groups_pt_oilgas_${STKCASEG}.ncf
  setenv STK_GRPS_007 $IN_PTpath/stack_groups_cmv_c3_12_${STKCASEG}.ncf
  setenv STK_GRPS_008 $IN_PTpath/stack_groups_cmv_c1c2_12_${STKCASEG}.ncf

  # Emission Rates for Inline Point Sources
  setenv grpIX 5
  #setenv iday `grep ${YYYYMMDD} ${EMISpath}/mergedates/smk_merge_dates_${YYYYMM}.txt | cut -d ',' -f$grpIX`
  set iday = `awk -F',' -v d=${YYYYMMDD} -v c=${grpIX} '$1 == d {gsub(/^[ \t]+|[ \t]+$/, "", $c); print $c}' ${EMISpath}/mergedates/smk_merge_dates_${YYYYMM}.txt`
  setenv STK_EMIS_001 $IN_PTpath/inln_mole_ptnonipm_${iday}_${STKCASEE}.ncf
  setenv STK_EMIS_002 $IN_PTpath/inln_mole_ptegu_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_003 $IN_PTpath/inln_mole_ptagfire_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_004 $IN_PTpath/inln_mole_ptfire-rx_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_005 $IN_PTpath/inln_mole_ptfire_othna_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_006 $IN_PTpath/inln_mole_pt_oilgas_${iday}_${STKCASEE}.ncf
  setenv STK_EMIS_007 $IN_PTpath/inln_mole_cmv_c3_12_${YYYYMMDD}_${STKCASEE}.ncf
  setenv STK_EMIS_008 $IN_PTpath/inln_mole_cmv_c1c2_12_${YYYYMMDD}_${STKCASEE}.ncf

  # Label Each Emissions Stream
  setenv STK_EMIS_LAB_001 PT_NONEGU
  setenv STK_EMIS_LAB_002 PT_EGU
  setenv STK_EMIS_LAB_003 PT_AGFIRES
  setenv STK_EMIS_LAB_004 PT_RXFIRES
  setenv STK_EMIS_LAB_005 PT_OTHFIRES
  setenv STK_EMIS_LAB_006 PT_OILGAS
  setenv STK_EMIS_LAB_007 PT_CMV_C3
  setenv STK_EMIS_LAB_008 PT_CMV_C1C2
  ...
  # DBP -->
  # If you are trying to use another date for a specific date file, turn it to true (T), but Be very careful not to set STK_EM_SYM_DATE_XXX = T on streams that:
  # Have exact date-based temporal profiles (e.g., holiday adjustments, weekly cycles)
  # Must match real dates (e.g., wildfires, episodic sources)
  # Symbolic mode repeats the data every model day as if it’s always the same date.
  ...
  #> In-line lightning NOx options - we may use both NLDN_STRIKES (re-gridded to d02) and LTNGPARMS_FILE
     setenv USE_NLDN  N        #> use hourly NLDN strike file [ default: Y ]
     if ( $USE_NLDN == Y ) then
        setenv NLDN_STRIKES ${IN_LTpath}/WWLLN_20230708/WWLLNs_12km_60min_${YYYYMMDD}.ioapi		#WWLLN files from: https://cmas-wwlln-lightning.s3.amazonaws.com/index.html#cmas-wwlln-lightning/2023/ -- DBP
     endif
     setenv LTNGPARMS_FILE ${IN_LTpath}/LTNG_AllParms_ChicagoLADCO_d02_20230708_windowed_from_12US1.ncf  #> lightning parameter file - Jordan
  endif

  #> In-line biogenic emissions configuration
  if ( $CTM_BIOGEMIS_BE == 'Y' ) then
     set IN_BEISpath = ${INPDIR}/surface
     #setenv GSPRO          $IN_BEISpath/gspro_beis_cmaq_cb6ae7_2017gb_17j_28apr2020.txt
     setenv GSPRO          ${WRF_DIR}/cmaq/gspro_biogenics.txt
     setenv BEIS_NORM_EMIS $IN_BEISpath/beis4_beld6_norm_emis_ChicagoLADCO_d02_windowed.ncf
     #setenv B3GRD      $IN_BEISpath/b3grd_2019ge_cb6_19k_ChicagoLADCO_d02_windowed_from_12US1.ncf
     setenv BIOG_SPRO   B10C6AE7 # speciation profile to use for biogenics - DBP: CB6r[3,5] - ae7 based mechanism
     #setenv BIOSW_YN    N     # use frost date switch [ default: Y ]
     #setenv BIOSEASON  $IN_BEISpath/bioseason.cmaq.19k_v2_ChicagoLADCO_d02_windowed_from_12US1.ncf #> ignore season switch file if BIOSW_YN = N
     ...
     setenv BEIS_SOILINP        $OUTDIR/CCTM_BSOILOUT_${RUNID}_${YESTERDAY}.nc  #> Biogenic NO soil input file; ignore if NEW_START = TRUE
  endif
  # Windblown dust emissions configuration
     # if ( $CTM_WB_DUST == 'Y' ) then
         # Input variables for BELD3 Landuse option
      #   setenv DUST_LU_1 $LUpath/GRIDCRO2D_ChicagoLADCO_d02_${YYYYMMDD_2}.nc
      #   setenv DUST_LU_2 $LUpath/GRIDCRO2D_ChicagoLADCO_d02_${YYYYMMDD_2}.nc
      #   setenv MODIS_FPAR $SZpath/MODIS_FPAR_LAI_daily_Chicago_LADCO_ioapi_d02_newWRF.nc

         # Input variables for BELD4 Landuse option
       # setenv BELD4_LU $SZpath/beld41_feb2017_ChicagoLADCO_d02_output.ncf
  ...
  setenv OCEAN_1 $SZpath/OCEAN_12_L3m_MC_CHL_chlor_ChicagoLADCO_d02_windowed_from_12US1.ncf3 #> horizontal grid-dependent ocean file - includes DMS and CHLO vars - DBP
...
# WRF-CMAQ Files
 ...
 #setenv XJ_DATA $JVALpath/$JVALfile
  ...
  #> mechanism name list files
      ln -s $NMLpath/GC_$MECH.nml             gc_matrix_nml
      ln -s $NMLpath/AE_$MECH.nml             ae_matrix_nml
      ln -s $NMLpath/NR_$MECH.nml             nr_matrix_nml
      ln -s $NMLpath/Species_Table_TR_0.nml  tr_matrix_nml
# ===================================================================
...
/
 ...
 eta_levels                          = 1.0, 0.9975, 0.995, 0.99,
                                       0.985, 0.98, 0.97, 0.96,
				       0.95, 0.94, 0.93, 0.92, 0.91,
				       0.9, 0.88, 0.86, 0.84, 0.82,
				       0.8, 0.77, 0.74, 0.7, 0.65,
 ...
 grid_fdda                           = 1,    
 grid_sfdda                          = 1,
 gfdda_inname                        = "wrffdda_d02",
 sgfdda_inname                       = "wrfsfdda_d02",
 pxlsm_soil_nudge                    = 1,
...
End_Of_Namelist
      ...
      rm -f wrfbdy_d01 wrffdda_d02 wrfsfdda_d02 wrfinput_d02 wrflowinp_d02
      ln -sf $METpath/wrfbdy_d02 wrfbdy_d01
      ln -sf $METpath/wrffdda_d02 wrffdda_d02
      ln -sf $METpath/wrfsfdda_d02 wrfsfdda_d02
      if (${WRF_RSTFLAG} == .false.) then
         ln -sf $METpath/wrfinput_d02 wrfinput_d01
      else if (${WRF_RSTFLAG} == .TRUE.) then
         ln -sf $METpath/wrfrst_d02* .
      endif
      ln -sf $METpath/wrflowinp_d02 wrflowinp_d02
...
# ===================================================================
  ...
  #time mpirun -verbose -np $NPROCS ${OUTDIR}/wrf.exe
  date
...
# rm -rf buff_${EXECUTION_ID}.txt
  ...
  if ($wrf_cmaq_option > 1) then
     if ( ! -e $OUTDIR/CCTM_CGRID_${CTM_APPL}.nc ) then   #commented out by DBP
       echo ""
       echo "**************************************************************"
